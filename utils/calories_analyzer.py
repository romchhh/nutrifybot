...
- Якщо конкретного продукту немає в базі — використовуй найближчий аналог з бази \
  і вкажи це в полі notes.
...
Формат відповіді (суворо JSON):
{
  ...
  "kb_used": true,
  "notes": "які продукти з бази використано, якщо є відхилення — вкажи"
}
"""
...
class CalorieAnalyzer:
    ...
    def _rag_search(self, query: str) -> list[dict]:
        ...
        """
        if not self._kb:
            logger.warning("CalorieAnalyzer: KB не ініціалізований")
            return []
        ...
        # Обрізаємо до максимуму щоб не перевантажувати prompt
        results = results[: self.RAG_MAX_CONTEXT_ITEMS]

        if results:
            names = [r["name"] for r in results]
            logger.info(
                "RAG знайдено %d результатів для '%s': %s",
                len(results),
                query[:50],
                ", ".join(names),
            )
        else:
            logger.warning("RAG: нічого не знайдено для '%s'", query[:50])
    ...
    async def _call_gpt(
        ...
        try:
            ...
            logger.info(
                "CalorieAnalyzer: '%s' — %.0f ккал | Б %.1f Ж %.1f В %.1f | KB=%s | conf=%s",
                result.name,
                result.calories,
                result.protein,
                result.fat,
            ...
            logger.error("CalorieAnalyzer GPT error: %s", exc)
            raise
